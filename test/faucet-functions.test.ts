  ...
  it('should test the pause() and unpause() functions', async function () {
    ...
    try {
      ...
      if (error.data) {
        ...
        if (decodedError) expect(decodedError.name).equals('WithdrawalPaused');
        else throw new error('Paused status should always raise a WithdrawalPaused custom error!');
      }
    }
    ...
    try {
      ...
      if (error.data) {
        ...
        if (decodedError)
          ...
          throw new error(
            'Unpaused status should raise a LowBalance custom error when we try the withdraw twice after just one deposit of 0.001 ETH!'
          );
      }
    }
  });
